...
percentage_data_2 <- ces2020_votes %>%
  group_by(birthyr) %>%
  summarise(
    `Joe Biden` = sum(voted_for == "Biden") / n() * 100,  # Enclosed in backticks
    DonaldTrump = sum(voted_for == "Trump") / n() * 100
  ) %>%
  ungroup()
...
# Plot for Joe Biden
plot_biden <- ggplot(percentage_data_2_long %>% filter(Candidate == "Joe Biden"), aes(x = birthyr, y = Percentage)) +
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  labs(title = "Percentage for Joe Biden", y = "Percentage (%)", x = "Birth Year") +  # Adding x-axis label
  theme_minimal()

plot_biden

# Plot for Donald Trump
plot_trump_2 <- ggplot(percentage_data_2_long %>% filter(Candidate == "DonaldTrump"), aes(x = birthyr, y = Percentage)) +  # Match summarise naming
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE, color = "red") +
